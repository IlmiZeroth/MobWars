...
public class PathfinderGoalMoveToLocation extends PathfinderGoal {
    ...
    public PathfinderGoalMoveToLocation(EntityInsentient entity, double x, double y, double z, double speed, BlockMob blockMob) {
        this.entity = entity;
        this.x = x;
        this.y = y;
        this.z = z;
...
//        System.out.println("a");
        ...
        if (distanceSquared < 3) { // Если ближе чем 1.73 блок (√3)
            ...
            Bukkit.getScheduler().runTaskLater(MobWars.getInstance(), () -> {
                entity.remove(Entity.RemovalReason.a, EntityRemoveEvent.Cause.PLUGIN); // Уничтожаем ентити
            }, 20);
