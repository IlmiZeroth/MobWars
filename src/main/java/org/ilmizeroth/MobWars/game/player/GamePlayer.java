...
import org.ilmizeroth.MobWars.gui.ChangeWeaponGUI;
import org.ilmizeroth.MobWars.gui.ChestGUI;
import org.ilmizeroth.MobWars.gui.GameTeamGUI;
import org.ilmizeroth.MobWars.mob.BlockMob;
import org.ilmizeroth.MobWars.mob.MobController;
...
public class GamePlayer {
    ...
    public boolean spawnBlockMob(BlockMob blockMob){
        if (canSpawn(blockMob)){
            this.playerBalance -= blockMob.getStats().cost();
            blockMobCounts.put(blockMob.getClass(), blockMobCounts.get(blockMob.getClass())-1);
            blockMob.spawnMob();
            summonRate--;
            playerPayDay += blockMob.getStats().paydayIncrease();
    ...
    private boolean canSpawn(BlockMob blockMob){
        if(blockMobCooldowns.containsKey(blockMob.getClass())){
            if(blockMobCooldowns.get(blockMob.getClass()) <= 1 || blockMobCounts.get(blockMob.getClass()) > 0){
                if(this.playerBalance < blockMob.getStats().cost()){
                    return false;
                } else {
